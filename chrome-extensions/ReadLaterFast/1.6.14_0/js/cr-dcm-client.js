...
function DiagnosticEventSender(context, subject, isAsync) {
    ...
    function getOffset() {
        var offset = parseInt(settings.get(ENGINE_SETTINGS.EPOCH_OFFSET));
...
function ClientInstallComponent(context) {
    ...
    //TODO: Extract update procedures
    function forceNewEngineDownload() {
...
function XhrDecorator(context, theXhrExecutor) {
    ...
    this.send = function(data, async) {
        xhrExecutor.setRequestHeader("X-Client-Version", "1.1.126");
...
dcm.addComponent("client/install", ["client/diagnostic", "client/version"], new ClientInstallComponent(dcm));
...
//        '            src="data:text/html;charset=utf-8,' + encodeURIComponent(config.text) + '"></iframe>' +
     ...
     * @param {Function} callback is called with dialog's result*/
...
function LoggerFactory(context) {
    ...
    var settings = context.storageFactory.getSettings();
    var fileAppender = new FileLogAppender(context.storageFactory.getStorage("dcm-log.txt"));
    ...
    this.getLogger = function(className) {
        ...
        return new Logger(className, LOG_LEVEL[loggerLevel.toUpperCase()], [fileAppender, consoleAppender]);
    }
}
